<template>
  <div class="min-h-screen px-6 pt-10 pb-8 flex flex-col">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-5">
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Nom <span class="text-red-800 font-bold">*</span></label>
        <input v-model="lastName" type="text" required class="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#8C0004]" />
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Prénoms <span class="text-red-800 font-bold">*</span></label>
        <input v-model="firstName" type="text" required class="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#8C0004]" />
      </div>
      <div>
      ...
      <button :disabled="!canSubmit" type="submit" class="w-full rounded-lg bg-[#8C0004] text-white py-3 text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed">Continuer</button>
    </form>
    <div class="mt-6 text-center text-sm">
        Déjà un compte ? 
      <router-link to="/login" class="text-[#8C0004] hover:underline">Connexion</router-link>
    </div>
  </div>
</template>
...
const canSubmit = computed(() => lastName.value && firstName.value && validPhone.value)

function handleSubmit() {
  if (!canSubmit.value) return
  store.startSignup(phoneLocal.value)
  router.push('/otp')
}
</script>
